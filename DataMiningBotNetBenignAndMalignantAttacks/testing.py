...
with open(
    ...
    # ConcatTraffic = [BenignTrafficFrame, BenignTrafficFrame]
...
# print(MalignTrafficFrameUDP.drop_duplicates(keep=False))
print(NewMalignTrafficFrameUDP)

# print(MalignTrafficFrameTCP.drop_duplicates(keep=False))
print(NewMalignTrafficFrameTCP)

print(MalignTrafficFrameScan)
print(MalignTrafficFrameJunk)
...
bucket = ([0] * len(BenignTrafficFrame.index) + [1] * len(MalignTrafficFrame.index) +
          [1] * len(NewMalignTrafficFrameUDP.index) + [1] * len(NewMalignTrafficFrameTCP.index) +
          [1] * len(MalignTrafficFrameScan.index) + [1] * len(MalignTrafficFrameJunk.index))

# bucket = ([0] * len(BenignTrafficFrame.index) + [1] * len(MalignTrafficFrame.index) +
#           [2] * len(MalignTrafficFrameUDP.index) + [3] * len(MalignTrafficFrameTCP.index) +
...
# param_grid = [{'C': [1, 10], 'break_ties': [False], 'coef0': [0.0], 'decision_function_shape': ['ovr'],
#                'gamma': [1], 'kernel': ['rbf'], 'shrinking': [False]},
#               {'C': [1, 10], 'gamma': [0.01, 0.001], 'kernel': ['poly'], 'shrinking': [False],
#                'decision_function_shape': ['ovr'], 'break_ties': [False], 'coef0': [1.0],
#                'degree': [3, 4]},
#               {'C': [1, 10], 'gamma': [0.001], 'kernel': ['poly'], 'shrinking': [False],
#                'decision_function_shape': ['ovr'], 'break_ties': [False], 'coef0': [1.0],
#                'degree': [5]}
#               ]

param_grid = {'C': [10, 1], 'break_ties': [False], 'coef0': [1.0], 'decision_function_shape': ['ovr'],
              'gamma': [10, 1], 'kernel': ['rbf'], 'shrinking': [False]}
